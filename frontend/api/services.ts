...
export const getServiceDetail = async (id: string): Promise<ServiceItem> => {
  try {
    ...
    if (!serviceDetail || typeof serviceDetail !== 'object') {
      console.warn('未获取到服务详情数据，使用模拟数据');
      // 模拟不同ID返回不同的服务详情
      const mockDetails: Record<string, ServiceItem> = {
        ...
        '3': {
          ...
          icon: 'Cloud',
          title: '云服务',
          description: '提供云计算解决方案，包括云迁移、云托管和云安全服务。我们与主流云服务提供商合作，为客户提供安全、稳定、高效的云服务。无论是公有云、私有云还是混合云，我们都能为您提供定制化的解决方案。',
          desc: '提供云计算解决方案，包括云迁移、云托管和云安全服务。'
        }
      };
      return mockDetails[id] || mockDetails['1']; // 如果没有对应ID的模拟数据，返回默认数据
    }
