...
export const getBannerList = async (): Promise<BannerItem[]> => {
  try {
    ...
    const banners: BannerItem[] = response
      ...
      .sort((aItem, bItem) => {
        // 从原始数据中获取order字段进行排序
        const originalA = response.data.find(item => item._id === aItem.id);
        const originalB = response.data.find(item => item._id === bItem.id);
        const orderA = originalA ? originalA.order : 0;
        const orderB = originalB ? originalB.order : 0;
