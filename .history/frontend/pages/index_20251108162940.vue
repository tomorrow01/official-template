  ...
  <div class="home-container" style="margin-top: 0; padding-top: 0; position: relative;">
    ...
    <section class="services-section py-12 bg-gray-50" style="margin-top: 0; margin-bottom: 0;">
      <div class="container">
        ...
        <!-- 服务卡片网格 - 使用明确的响应式类确保大屏幕显示三个卡片 -->
        <div style="display: grid; grid-template-columns: 1fr; gap: 2rem;" class="lg:grid-cols-3 md:grid-cols-2">
          <!-- 每个服务项是一个独立卡片 -->
          <a 
            v-for="(service, index) in services" 
            :key="service.id" 
            href="/services" 
            style="display: flex; flex-direction: column; background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); transition: all 0.3s ease;"
            class="hover:shadow-xl transform hover:-translate-y-1"
          >
            <!-- 卡片内部：图片在上 -->
            ...
            <div style="padding: 1.5rem;">
              <!-- 标题 -->
              <h3 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 0.75rem;">{{ service.title }}</h3>
              ...
              <!-- 跳转到详情页面的链接 -->
              <div style="display: flex; align-items: center; color: #0d6efd; font-weight: 500;">
