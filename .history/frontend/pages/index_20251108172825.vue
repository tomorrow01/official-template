  ...
  <div class="home-container" style="margin-top: 0; padding-top: 0; position: relative;">
    ...
    <section class="services-section py-12 bg-gray-50" style="margin-top: 0; margin-bottom: 0;">
      <div class="container">
        ...
        <!-- 服务卡片网格 - 响应式布局：大屏3列，中屏2列，小屏1列 -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" style="display: grid !important; grid-template-columns: repeat(1, 1fr) !important;">
          <div v-bind:style="{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }" v-if="$q.screen.lg">
          ...
          <template v-else-if="$q.screen.md">
            <template v-for="(service, index) in services" :key="service.id">
              <a 
                href="/services" 
                class="md:col-span-1 flex flex-col bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1"
              >
                <!-- 卡片内部：图片在上 -->
                <div class="h-52 overflow-hidden">
                  <img 
