  ...
  <div class="home-container" style="margin-top: 0; padding-top: 0; position: relative;">
    ...
    <section class="services-section py-12 bg-gray-50" style="margin-top: 0; margin-bottom: 0;">
      <div class="container">
        ...
        <!-- 服务卡片网格 - 每行三张卡片 -->
        <div class="services-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          <!-- 每个服务项是一个独立卡片 -->
          <a 
            v-for="(service, index) in services" 
            :key="service.id" 
            href="/services" 
            :class="`service-card-block bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden`"
            :style="{animationDelay: `${index * 100}ms`}"
          >
            <!-- 卡片内部：图片在上 -->
            <div class="service-image-block h-52 overflow-hidden">
              <img 
            ...
            <div class="p-6">
              <!-- 标题 -->
              <h3 class="service-title-block text-xl font-semibold mb-3">{{ service.title }}</h3>
              
              <!-- 详情 -->
              <p class="service-desc-block text-gray-600 mb-4">
                {{ service.desc || '为客户提供专业的技术解决方案，助力业务增长和数字化转型。' }}
              </p>
              
              <!-- 跳转到详情页面的链接 -->
              <div class="service-detail-link flex justify-start items-center text-primary font-medium">
