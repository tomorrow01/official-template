...
const handleSubmit = async () => {
  ...
  try {
    if (currentId.value) {
      // 编辑：更新现有数据
      const response = await axios.put(`/api/services/${currentId.value}`, form.value);
      if (response.data.code === 200) {
      ...
      // 新增：生成唯一ID并添加数据
      const response = await axios.post('/api/services', form.value);
...
const handleDelete = async (row) => {
  try {
    ...
    // 调用API删除
    const response = await axios.delete(`/api/services/${row._id || row.id}`);
    if (response.data.code === 200) {
      services.value = services.value.filter(item => item._id !== row._id && item.id !== row.id);
...
const handleStatusChange = async (row) => {
  try {
    const response = await axios.put(`/api/services/${row._id || row.id}`, {
