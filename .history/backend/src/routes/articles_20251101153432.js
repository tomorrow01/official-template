...
router.put('/:id', async (req, res) => {
  try {
    console.log('收到更新文章请求，ID:', req.params.id);
    console.log('更新数据:', req.body);
    
    if (isMongoConnected()) {
      console.log('使用MongoDB更新文章');
      const article = await Article.findByIdAndUpdate(req.params.id, req.body, { new: true });
      
      if (!article) {
        console.log('未找到文章，ID:', req.params.id);
        return sendResponse(res, 404, null, '文章不存在');
      }
      
      console.log('文章更新成功，ID:', req.params.id);
      sendResponse(res, 200, article, '文章更新成功');
    } else {
      // 数据库连接失败，返回模拟更新成功的消息
      console.log('使用模拟数据模式更新');
